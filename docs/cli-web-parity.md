...
| `--verify-repack` / `--repack-bits` | yes | `POST /api/jobs/verify-repack?left=…&right=…&<scope>[&repack_bits=N]`, polled. Candidate pairs, bit width and the "does anything else differ" question come from `compare::plan_repack`, shared with the CLI; decoding is `remote::verify_repack` on the proxy for an `s3://` pair, or the CLI's own `local_repack` for local files. Which pairs can run at all is `compare::repack_supported`, asked by both surfaces **before** either side is read — the CLI used to ask after two structure reads and a printed diff, so a `:/path` + `s3://` pair looked accepted. Verified against a real pair: 2 pairs, 27 GB read on the remote, 318 s |
